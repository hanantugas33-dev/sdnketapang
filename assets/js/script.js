  ...
  try {
    ...
    if (data.fasilitas && data.fasilitas.length) {
      list.innerHTML = data.fasilitas.map(f => {
        // Kalau ada foto, tampilkan kartu dengan gambar
        if (f.foto) {
          return `
            <div class="fas-item fas-item-foto">
              <div class="fas-foto">
                <img src="assets/img/fasilitas/${f.foto}" alt="${f.nama}" loading="lazy"/>
              </div>
              <div class="fas-txt">
        ...
        // Fallback: tampil dengan emoji icon
        return `
          <div class="fas-item">
            <div class="fas-icon">${f.icon}</div>
            <div class="fas-txt">
              <strong>${f.nama}</strong>
              <span>${f.deskripsi || ''}</span>
            </div>
    ...
    if (data.ekskul && data.ekskul.length) {
      chips.innerHTML = data.ekskul.map(e => {
        // Kalau ada foto, tampilkan chip dengan gambar kecil
        const fotoEl = e.foto
