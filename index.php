...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_fas'])) {
    try {
        ...
        if (!$nama) throw new Exception("Nama fasilitas wajib diisi!");
        ...
        if ($id) {
            $db->prepare("UPDATE fasilitas SET nama=?,deskripsi=?,foto=?,jumlah=?,kondisi=?,urutan=? WHERE id=?")
               ->execute([$nama,$desk,$foto,$jml,$kond,$urut,$id]);
        } else {
            $db->prepare("INSERT INTO fasilitas (nama,deskripsi,foto,jumlah,kondisi,urutan) VALUES (?,?,?,?,?,?)")
               ->execute([$nama,$desk,$foto,$jml,$kond,$urut]);
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_eks'])) {
    try {
        ...
        if (!$nama) throw new Exception("Nama ekskul wajib diisi!");
        ...
        if ($id) {
            $db->prepare("UPDATE ekskul SET nama=?,foto=?,hari=?,jam_mulai=?,jam_selesai=?,pembina=?,deskripsi=?,urutan=? WHERE id=?")
               ->execute([$nama,$foto,$hari,$jam_m,$jam_s,$pembina,$desk,$urut,$id]);
        } else {
            $db->prepare("INSERT INTO ekskul (nama,foto,hari,jam_mulai,jam_selesai,pembina,deskripsi,urutan) VALUES (?,?,?,?,?,?,?,?)")
               ->execute([$nama,$foto,$hari,$jam_m,$jam_s,$pembina,$desk,$urut]);
        }
...
<?php if ($msg): ?>
<div class="alert <?= str_starts_with($msg,'❌')?'alert-error':'alert-success' ?>"><?= $msg ?></div>
<?php endif; ?>
    ...
    <div class="card-body">
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="text" name="nama_fas" value="<?= htmlspecialchars($editFas['nama']??'') ?>"
                 required placeholder="Contoh: Ruang Kelas"/>
        </div>

        <div class="form-group">
          <label>Deskripsi Singkat</label>
          <input type="text" name="desk_fas" value="<?= htmlspecialchars($editFas['deskripsi']??'') ?>"
                 placeholder="Contoh: 12 ruang kelas berventilasi baik" maxlength="200"/>
        </div>

        <!-- UPLOAD FOTO FASILITAS -->
        <div class="form-group">
          <label>📷 Foto Fasilitas <span style="color:#c62828;font-size:11px;font-weight:400">* Wajib upload foto</span></label>

          <?php if (!empty($editFas['foto'])): ?>
          <div style="margin-bottom:10px;">
            <img src="../assets/img/fasilitas/<?= htmlspecialchars($editFas['foto']) ?>"
                 style="width:100%;max-height:150px;object-fit:cover;border-radius:10px;display:block;border:1px solid #eee;"/>
            <div style="margin-top:6px;display:flex;align-items:center;gap:8px;">
              <input type="checkbox" name="hapus_foto_fas" value="1" id="hapusFas"/>
              <label for="hapusFas" style="font-size:12px;color:#c62828;cursor:pointer;font-weight:500;">🗑️ Hapus foto ini</label>
            </div>
          </div>
          ...
          <input type="file" id="fotoFasInput" name="foto_fas"
                 ...
                 style="display:none" onchange="previewImg(this,'prevFas')"/>
        </div>
      ...
      <tbody>
        ...
        <tr>
          <td>
            ...
            <?php else: ?>
              <div style="width:64px;height:48px;background:#f5f5f5;border-radius:8px;
                          display:flex;align-items:center;justify-content:center;font-size:22px;color:#bbb;">📷</div>
            <?php endif; ?>
          </td>
    ...
    <div class="card-body">
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <input type="text" name="nama_eks" value="<?= htmlspecialchars($editEks['nama']??'') ?>"
                 required placeholder="Contoh: Pramuka"/>
        </div>

        <!-- UPLOAD FOTO EKSKUL -->
        <div class="form-group">
          <label>📷 Foto Ekskul <span style="color:#c62828;font-size:11px;font-weight:400">* Wajib upload foto</span></label>

          <?php if (!empty($editEks['foto'])): ?>
          <div style="margin-bottom:10px;">
            <img src="../assets/img/ekskul/<?= htmlspecialchars($editEks['foto']) ?>"
                 style="width:100%;max-height:130px;object-fit:cover;border-radius:10px;display:block;border:1px solid #eee;"/>
            <div style="margin-top:6px;display:flex;align-items:center;gap:8px;">
              <input type="checkbox" name="hapus_foto_eks" value="1" id="hapusEks"/>
              <label for="hapusEks" style="font-size:12px;color:#c62828;cursor:pointer;font-weight:500;">🗑️ Hapus foto ini</label>
            </div>
          </div>
          <div style="font-size:12px;color:#999;margin-bottom:6px;">Upload baru untuk mengganti.</div>
          <?php endif; ?>
          ...
          <input type="file" id="fotoEksInput" name="foto_eks"
                 accept="image/jpeg,image/png,image/webp,image/gif"
                 style="display:none" onchange="previewImg(this,'prevEks')"/>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="text" name="pembina_eks" value="<?= htmlspecialchars($editEks['pembina']??'') ?>"
                 placeholder="Contoh: Pak Ahmad Fauzi"/>
        </div>
        <div class="form-group">
          <label>Deskripsi (Opsional)</label>
          <textarea name="desk_eks" rows="2" placeholder="Deskripsi singkat..."><?= htmlspecialchars($editEks['deskripsi']??'') ?></textarea>
        </div>
        <div class="form-group">
          <label>Urutan</label>
...
.upload-zone{
  ...
  cursor:pointer;transition:all .2s;background:#fafafa;margin-bottom:0;
}
